require 'nn'
local DSSM_CosineDist, parent = torch.class('nn.DSSM_CosineDist', 'nn.Module')

-- the cos similarity function take two paired vector and negtive sample index as input.
function DSSM_CosineDist:__init()
    parent.__init(self)
    self.gradInput = {torch.Tensor(), torch.Tensor()}
end

function DSSM_CosineDist:updateOutput(input)
    local input1, input2, Index = input[1], input[2], input[3]

    input1 = input1:index(1, Index)
    input2 = input2:index(1, Index)
...
function DSSM_CosineDist:updateGradInput(input, gradOutput)
   local v1  = input[1]
   local v2  = input[2]
